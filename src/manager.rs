...
impl Manager {
    ...
    async fn _run_quorum(
        ...
        let response = self._quorum_with_retries(timeout, lighthouse_request).await?;
    ...
    async fn _quorum_with_retries(&self, timeout: Duration, lighthouse_request: LighthouseQuorumRequest) -> Result<tonic::Response<LighthouseQuorumResponse>, Status> {
        let mut client = self.lighthouse_client.clone();
        ...
        loop {
            ...
            let result = tokio::time::timeout(timeout, client.quorum(request))
                .await;
