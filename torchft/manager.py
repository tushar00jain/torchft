    ...
    def allreduce(
        ...
        * The tensor must be zeroed before being used as it may be corrupted.
        ...
        try:
            ...
            def callback(tensors: list[torch.Tensor]) -> None:
                nonlocal num_participants
                tensors[0] /= num_participants
...
class _ManagedFuture(torch.futures.Future[list[torch.Tensor]]):
    ...
    def then(
        self, callback: Callable[[_SimpleFuture], Any]
    ) -> torch.futures.Future[Any]:
        self._work.add_callback(callback)
        return self
...
class _ManagedWork(dist._Work):
    def __init__(
        ...
        self._callbacks: list[Callable[[_SimpleFuture], None]] = []

    def add_callback(self, callback: Callable[[_SimpleFuture], None]) -> None:
    ...
    def _set_future_callback(
        ...
        while self._callbacks:
            user_callback: Callable[[_SimpleFuture], None] = self._callbacks.pop(0)
            ...
            def callback(
                ...
                with (
                    ...
                    fut.wait()
                    user_callback(_SimpleFuture(self._tensors))
                    return self._tensors
