...
class Manager:
    ...
    def allreduce(
        ...
        * The tensor must be zeroed before being used as it may be corrupted.
        ...
        try:
            ...
            # TODO: support `wrap_future` so that it also doesn't
            # make the callback stream dependent on the process group's
            # stream running allreduce
    ...
    def should_commit(self, timeout: Optional[timedelta] = None) -> bool:
        ...
        with torch.profiler.record_function("torchft::manager::should_commmit::recovery_stream::synchronize"):
            if self._recovery_stream is not None:
                self._recovery_stream.synchronize()

        with torch.profiler.record_function("torchft::manager::should_commit::current_stream::synchronize"):
